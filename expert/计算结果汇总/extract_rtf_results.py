...
COM_RETRY_DELAY = 0.2
WORD_QUIT_WAIT = 0.8
# Word.Application typelib（损坏的 gen_py 缓存常导致 CLSIDToPackageMap 错误）
WORD_TYPLIB_MARKER = "00020905"
...
def _clear_corrupt_word_gencache() -> None:
    """删除损坏的 Word gen_py 缓存（CLSIDToPackageMap / CLSIDToClassMap）。"""
    import os
    ...
    if os.path.isdir(gen_path):
        for name in os.listdir(gen_path):
            if WORD_TYPLIB_MARKER in name.upper():
                shutil.rmtree(os.path.join(gen_path, name), ignore_errors=True)
    for mod_name in list(sys.modules.keys()):
        if "gen_py" in mod_name and WORD_TYPLIB_MARKER in mod_name:
            del sys.modules[mod_name]


def _is_word_gencache_error(exc: BaseException) -> bool:
    msg = str(exc)
    return (
        "CLSIDToPackageMap" in msg
        or "CLSIDToClassMap" in msg
        or "gen_py" in msg
    )
...
def create_word_app():
    ...
    for attempt in range(3):
        try:
            ...
            if _is_word_gencache_error(exc) and attempt < 2:
                _clear_corrupt_word_gencache()
                continue
            raise
    return win32com.client.dynamic.Dispatch("Word.Application")
...
def convert_rtf_batch(
    ...
    try:
        for rtf in ordered_rtfs:
            ...
            try:
                out = convert_rtf_to_named_docx(
                    rtf, out_stem, word, append_title=append_title,
                )
...
def convert_rtfs_to_docx(
    ...
    return convert_rtf_batch(ordered_rtfs, stem_to_id, append_title=append_title)
