"""
双轴弯矩版荷载表坐标校准（MY / MZ / 右侧下拉按钮）

用法：
  python expert_try_biaxial.py --calibrate
  python expert_try_biaxial.py --test-coords
"""
import argparse
import json
import os
...
COORDS_FILE = os.path.join(SCRIPT_DIR, "load_coords_biaxial.json")
...
def screen_points(gl, gt, coords):
    ...
    my_x = coords.get("LOAD_MY_COL_X", n_x + 55)
    mz_x = coords.get("LOAD_MZ_COL_X", my_x + 55)
    exp_x = gl + coords.get("LOAD_EXPAND_BTN_X", mz_x + 40)
...
def run_calibrate():
    ...
    coords = {
        ...
        "LOAD_N_COL_X": x_n - gl,
        "LOAD_MY_COL_X": x_my - gl,
        "LOAD_MZ_COL_X": x_mz - gl,
    ...
    for k, v in coords.items():
        print(f"  {k} = {v}")
    print("\n运行测试确认：  python expert_try_biaxial.py --test-coords")
    return coords, (gl, gt)
...
def run_test():
    ...
    coords = load_coords()
    if not coords:
        print(f"未找到 {COORDS_FILE}，请先运行: python expert_try_biaxial.py --calibrate")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Expert 双轴弯矩荷载表坐标校准")
