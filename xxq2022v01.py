...
dict_cad_id = {'1------AutoCAD2014------1': 19.1,
               '2------AutoCAD2016------2': 20,
               '3------AutoCAD2018------3': 22,
               '4------AutoCAD2020------4': 23.1,
               '5------AutoCAD2021------5': 24
               }
...
prog_id = 'AutoCAD.Application.22'


def show(event):
    global prog_id
    version_cad = lg.version.get()
    prog_id = 'AutoCAD.Application.' + str(dict_cad_id[version_cad])
    print(prog_id)


version = lg.com_version.bind('<<ComboboxSelected>>', show)

Layer_kuang, Layer_biaoge, Layer_jiegou, Layer_biaozhu, Layer_midline, Layer_elevation, doc, msp = 'x', 'x', 'x', 'x', 'x', 'x', 'x', 'x'


def cad_connect(prog_id):
    global Layer_kuang, Layer_biaoge, Layer_jiegou, Layer_biaozhu, Layer_midline, Layer_elevation, doc, msp
    try:
...
class pou():
    ...
    def __init__(self, hyi, scale, theta=0, pou_slope=1.5, pou_left_l=1.5, pou_right_l=1.5, pou_dingkuan=0.5,
                 pou_jinbian=0.2):
...
def main():
    global pou2, pou3, pou4, pou5

    scale = 10
    print(lg.excel_real_path)

    excel_path = lg.excel_real_path
    data = xlrd.open_workbook(excel_path).sheet_by_name("箱形桥输入数据")
    ...
    afa2 = int(data_input[30])
    pou_slope = data_input[27]
    d3, x_guilu = [float(100 * i) for i in data_input[2:4]]
    w0, l0, d1, d2, l_lei_up, h_lei_up, l_lei_down, h_lei_down, h0, d_le, thk_pu, d4, jichu_xbian = [float(100 * i) for
                                                                                                     i in
                                                                                                     data_input[5:18]]
    ...
    kongshu = float(data.col_values(4, 1, data.nrows + 1)[7])
    mid_d = float(data.col_values(4, 1, data.nrows + 1)[8]) * 100 / scale
    ...
    pou2 = pou(pou2_h, scale, theta, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
    pou4 = pou(pou4_h, scale, 0, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
    pou3 = pou(pou3_h, scale, theta, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
    pou5 = pou(pou5_h, scale, 0, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
    ...
    def lwpoly_huizhi(mubiao1):
        ...
        for i in mubiao1:
            if i not in x and len(i) == 2:
                ...
                x.append(i)
        mubiao1 = [j for i in x for j in i]
        polyobj = msp.AddLightWeightPolyline(vtfloat(mubiao1))
        return polyobj
    ...
    def hui_xiang(x0, y0, l0, h0, d_le, d1, d2, d3, theta):
        ...
        if kongshu > 1:
            x8_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t, y0 + d2 + h_lei_down]
            x9_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t, y0 + h0 - h_lei_up + d2]
            x12_1 = [x0 + (l0 + d_le - (l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t, y0 + h0 + d2 - h_lei_up]
            x13_1 = [x0 + (l0 + d_le - (l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t, y0 + d2 + h_lei_down]
            x10_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu - l_lei_up) / cos_t, y0 + h0 + d2]
            x7_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu - l_lei_up) / cos_t, y0 + d2]
            x11_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu + l_lei_up + mid_d * (kongshu - 1)) / cos_t,
                     y0 + h0 + d2]
            x14_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu + l_lei_up + mid_d * (kongshu - 1)) / cos_t,
                     y0 + d2]
            poly_huizhi([x7, x7_1, x8_1, x9_1, x10_1, x10, x9, x8, x7])
            poly_huizhi([x14_1, x14, x13, x12, x11, x11_1, x12_1, x13_1, x14_1])
            line_biaozhu(x8, x8_1, b1, theta=theta, bu_ping=False)
            line_biaozhu(x13, x13_1, b1, theta=theta, bu_ping=False)
            line_biaozhu(x8_1, x13_1, b1, theta=theta, bu_ping=False)
        else:
    ...
    def limian_yiqiang(x0, y0, l0, d_le, d2, theta, afa, right):
        if kongshu > 1:
            l0 = l0 * kongshu + (kongshu - 1) * mid_d
        # print('第643行', pou3.hyi)
        global pou3, pou5
        print('第645行', pou3.hyi)
        cos_t = cos(pi * theta / 180)
    ...
    def cemian_huizhi(x00, y00, w0, h0, h_lei_up, h_lei_down, d1, d2, d3, theta):
        ...
        y21 = [
            x00 - (pou2.hyi - pou4.hyi) * 1.5 - pou4.dingkuan - yanshen - fenxi + pou4.hyi * pou4.slope + pou4.dingkuan,
            y00 + pou4.jidi_h]
        ...
        zp4 = [
            x00 - (pou2.hyi - pou4.hyi) * 1.5 - pou4.dingkuan - yanshen - fenxi - (
                    pou4.hyi - pou4.dingkuan / 1.15) * 1.5,
            y00 + pou4.jidi_h]
        ...
        zp8 = [x00 - (pou2.hyi - pou4.hyi) * 1.5 - pou4.dingkuan - yanshen - fenxi - (
                pou4.hyi - pou4.dingkuan / 1.15 - zhuiti1 - zhuiti2) * 1.5 - 20 / scale,
               y00 + pou4.jidi_h - zhuiti_shen]
    ...
    def pingmian_huizhi(x00, y00, l0, w0, d_lef, theta, afa):
        ...
        x14 = [
            x00 - fenxi / cos_t - pou2.right_l + l0 / cos_t - pou3.right_l - fenxi / cos_t + (jin_bian + fenxi) * tan_t,
            y00 - jin_bian]
        ...
        xiangti = poly_huizhi(
            [x4, x3, x2, x5, x2, x1, x6, x1, x0, x15, x14, x13, x0, x12, x7, x12, x11, x8, x11, x10, x9])
        if kongshu > 0:
            x1_1 = [x00 - fenxi / cos_t - pou2.right_l + ((l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t, y00]
            x6_1 = [
                x00 - fenxi / cos_t - pou2.right_l - w0 * tan_t / 2 + ((l0 - mid_d * (kongshu - 1)) / kongshu) / cos_t,
                y00 + w0 / 2]
            x7_1 = [x00 - fenxi / cos_t - pou2.right_l - w0 * tan_t / 2 + (
                    ((l0 - mid_d * (kongshu - 1)) / kongshu) + mid_d) / cos_t, y00 + w0 / 2]
            x14_1 = [x00 - fenxi / cos_t - pou2.right_l + (((l0 - mid_d * (kongshu - 1)) / kongshu) + mid_d) / cos_t,
                     y00]
            line_hidden(line_huizhi(x1_1, x6_1), scale=1.5)
            line_hidden(line_huizhi(x7_1, x14_1), scale=1.5)
            biaozhu1 = [x4, x5, x6, x6_1, x7_1, x7, x8, x9]
        ...
        def yiqiang(left):
            ...
            Y21 = [x00 + fenxi * (tan_t - cos_t) - pou2.right_l - pou2.dingkuan - yanshen * tan_t,
                   y00 - fenxi - yanshen]
            ...
            z19 = [
                z21[0] - ((pou2.hyi - pou4.hyi) * 1.5 - pou4.hyi * pou4.slope) * tan_a - 1.264 * pou4.hyi * pou4.slope,
                z18[1]]
            ...
            biaozhu_delete = [bz1, bz2, bz3, bz4, bz5, bz6, bz7, bz8, bz9, bz10, bz11, bz12, bz13, bz14, bz15, bz16,
                              bz17,
                              zkb1, zkb2, zkb3] + jiemian1 + jiemian2

            zuoyi = poly_huizhi(
                [z7, z8, z9, z10, z9, z20, z21, z13, z21, z10, z11, z22, z11, z12, z12, z13, z12, z1, z2])
    ...
    def huizhi_shuliangbiao(ping_l, x00, y00):
        ...
        bridge_excel = xlrd.open_workbook(excel_path).sheet_by_name("Sheet1")
        ...
        bridge_excel2 = xlrd.open_workbook(excel_path).sheet_by_name("Sheet2")
        ...
        for j in range(len(table_x)):
            ...
            if j > 139:
                print("第一个数为：", table_text[j])
                lg.shuliang.append(table_text[j])
        ...
        v_jiangqs = 2 * round(
            cal_v_zhuiti(0.35, pou4.hyi * scale / 100)[0] + cal_v_zhuiti(0.35, pou5.hyi * scale / 100)[0],
            2)
        v_suishi = 2 * round(
            cal_v_zhuiti(0.1, pou4.hyi * scale / 100)[0] + cal_v_zhuiti(0.1, pou5.hyi * scale / 100)[0], 2)
        v_hangtian = ((2 * round(
    ...
    def xian_j(p, q_w=1.5/scale):
        x1 = (p[0] - 0.75) / scale
        x2 = (p[0] + 0.75) / scale
        p = [x1, p[1]/scale, x2, p[1]/scale]
        x = msp.AddLightWeightPolyline(vtfloat(p))
        print("down")
        x.ConstantWidth = q_w
        x.Closed = True
        x.SetBulge(0, 1)
    ...
    def gangjin_tu(x0, y0, l0, h0, d_le, d1, d2, d3, theta):
        ##  通过内部程序生成钢筋图
        doc.ActiveLayer = Layer_biaozhu
        if kongshu > 1:
            l0 = l0 * kongshu + (kongshu - 1) * mid_d
        ...
        x2 = [x0, y0 + h0 + d1 + d2]
        x3 = [x0 + (2 * d_le + l0)/2 / cos_t, y0 + h0 + d1 + d2]
        x4 = [x0 + (2 * d_le + l0)/2 / cos_t, y0]
        poly_huizhi([x1, x4, x3, x2])

        x7 = [x0, y0 + d2]
        x10 = [x0, y0 + h0 + d2]
        x11 = [x0 + (l0/2 - l_lei_up) / cos_t, y0 + h0 + d2]
        x12 = [x0 + (l0/2) / cos_t, y0 + h0 + d2 - h_lei_up]
        x13 = [x0 + (l0/2) / cos_t, y0 + d2 + h_lei_down]
        x14 = [x0 + (l0/2 - l_lei_down) / cos_t, y0 + d2]

        poly_huizhi([x7, x14, x13, x12, x11, x10])
        doc.ActiveLayer = Layer_jiegou
        for i in range(10):
            p = [x0 + i*20, y0+3.5]
            xian_j(p)
        ...
        #     x7_1 = [x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu - l_lei_up) / cos_t, y0 + d2]
        #     x11_1 = [
        #         x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu + l_lei_up + mid_d * (kongshu - 1)) / cos_t,
        #         y0 + h0 + d2]
        #     x14_1 = [
        #         x0 + (d_le + (l0 - mid_d * (kongshu - 1)) / kongshu + l_lei_up + mid_d * (kongshu - 1)) / cos_t,
        #         y0 + d2]
        #     poly_huizhi([x7, x7_1, x8_1, x9_1, x10_1, x10, x9, x8, x7])
        #     poly_huizhi([x14_1, x14, x13, x12, x11, x11_1, x12_1, x13_1, x14_1])
        #     line_biaozhu(x12, x8_1, b1, theta=theta, bu_ping=False)
        #     line_biaozhu(x13, x13_1, b1, theta=theta, bu_ping=False)
        #     line_biaozhu(x8_1, x13_1, b1, theta=theta, bu_ping=False)
        # else:
        #     poly_huizhi([x7, x12, x9, x10, x11, x12, x13, x14, x7])
        #
        #     line_biaozhu(x8, x12, b1, theta=theta, bu_ping=False)
        #
        # line_biaozhu(x1, x12, b1, theta=theta, bu_ping=False)
        # line_biaozhu(x12, x4, b1, theta=theta, bu_ping=False)
        #
        # b2 = [x9[0], x9[1] - 2 * biao_ju]
        # line_biaozhu(x9, x10, b2, theta=theta, bu_ping=False)
    ...
    def shuchu(lg, string):
        if lg.shuru_way =='zhu':
            lg.text_out.insert(lg.tkend, string)
            lg.text_out.update()
        else:
            lg.text_out1.insert(lg.tkend, string)
            lg.text_out1.update()
    ...
    if lg.gouzao == 'True':
        ...
        # doc.SendCommand('app')
        # print("end doing")
        # shuchu(lg, "0---------------程序数据初始化完成！\n")
        print('111', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        xiang_l = hui_xiang(980 / scale, 1720 / scale, l0, h0, d_le, d1, d2, d3, theta)  # 前两个是定位点，后面是
        print('222', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        shuchu(lg, "1---------------立面图箱体绘制完成！\n")
        yiliang_l = limian_yiqiang(980 / scale, 1720 / scale, l0, d_le, d2, theta, afa, right)
        print('333', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        limian_yiqiang(980 / scale, 1720 / scale, l0, d_le, d2, theta, afa, False)
        print('444', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        shuchu(lg, "2---------------立面图翼墙绘制完成！\n")
        cel_l = cemian_huizhi(1400 / scale, 500 / scale, w0, h0, h_lei_up, h_lei_down, d1, d2, d3, theta)
        print('555', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        shuchu(lg, "3---------------侧视图绘制完成！\n")
        ...
        shuchu(lg, "5---------------数量表计算并绘制完成！\n")
        pou2 = pou(pou2_h, scale, theta, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
        pou4 = pou(pou4_h, scale, 0, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
        pou3 = pou(pou3_h, scale, theta, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
        pou5 = pou(pou5_h, scale, 0, pou_slope, pou_left_l, pou_right_l, pou_dingkuan, pou_jinbian)
        print('p0', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)

        t2 = hui_pou(980 / scale + xiang_l + 1.99 * yiliang_l, 1720 / scale, pou3, "Ⅱ--Ⅱ 剖 面")
        print('p1', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        t3 = hui_pou(980 / scale + xiang_l + 1.99 * yiliang_l + t2, 1720 / scale, pou2, "Ⅲ--Ⅲ 剖 面")
        print('p2', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        t4 = hui_pou(980 / scale + xiang_l + 1.99 * yiliang_l + t2 + t3, 1720 / scale, pou4, "Ⅳ--Ⅳ 剖 面")
        print('p3', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        t5 = hui_pou(980 / scale + xiang_l + 1.99 * yiliang_l + t2 + t3 + t4, 1720 / scale, pou5, "Ⅴ--Ⅴ 剖 面")
        print('p4', pou2.di_chang, pou3.di_chang, pou4.di_chang, pou5.di_chang)
        shuchu(lg, "6---------------翼墙剖面绘制完成！\n")
        hui_tukuang(24.7 + cel_l[0] + 1600 / scale + ping_l[0] + 320 + 20)
        shuchu(lg, "7---------------图长计算完成！\n绘图成功！谢谢使用！\n")
        MessageBox(0, "绘图成功！谢谢使用！", "提醒", MB_OK)
    else:
        xiang_l = gangjin_tu(980 / scale, -1720 / scale, l0, h0, d_le, d1, d2, d3, theta)  # 前两个是定位点，后面是


values = [lg.xiangti_licheng_qian, lg.xiangti_licheng, lg.licheng_fangxiang, lg.zuo_li, lg.you_li, lg.guimian_ele,
          lg.guimian_podu, lg.guimian_xiangding, lg.guimian_lujian, lg.xiangti_theta, lg.kongshu, lg.xiangti_kongjin,
          lg.xiangti_qiaokuan, lg.xiangti_jinggao, lg.xiangti_dingban, lg.xiangti_diban, lg.xiangti_midfu,
          lg.xiangti_bianfu, lg.xiangti_dingdao_c, lg.xiangti_dingdao_g, lg.xiangti_didao_c, lg.xiangti_didao_g,
          lg.xiangti_dingpu, lg.xiangti_jichuhou, lg.xiangti_jichuchao, lg.yiqiang_afa, lg.yiqiang_afa2,
          lg.yiqiang_22, lg.yiqiang_44, lg.yiqiang_33, lg.yiqiang_55, lg.yiqiang_di, lg.yiqiang_dingkuan,
          lg.yiqiang_dingmaoshi, lg.yiqiang_di_zuo, lg.yiqiang_di_you, lg.yiqiang_podu, lg.yiqiang_fangpo,
          lg.mid_chengjiang, lg.mid_num, lg.xiangti_yanshen]

i_s = [1, 1, 2, 3, 4, 1, 2, 3, 4, 5, 8, 7, 6, 14, 8, 9, 9, 15, 10, 11, 12, 13, 16, 17, 18, 29, 31, 20, 21, 22, 23, 19,
       24, 25, 26, 27, 28, 30, 5, 6, 7]
j_s = [4, 5, 4, 4, 4, 2, 2, 2, 2, 2, 4, 2, 2, 2, 2, 2, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2,
       4, 4, 4]


def exzample_input():
    # global values, i_s, j_s
    data_exzample = xlrd.open_workbook('.\\example.xls').sheet_by_name("箱形桥输入数据")
    for i in range(len(i_s)):
        data = data_exzample.cell(i_s[i], j_s[i]).value
        values[i].set(str(data))
...
def read_zhuxiang():
    ...
    wbook = copy(workbook)
    wsheet = wbook.get_sheet(5)
    style_float = xlwt.easyxf(num_format_str='0.00')  # 自定义格式-显示两位数（四舍五入）
    style_int = xlwt.easyxf(num_format_str='0')  # 自定义格式-显示整数（四舍五入）
    style1 = xlwt.easyxf(num_format_str='general')

    for i in range(len(i_s)):
        if i < 2 or 2 < i < 5:
            wsheet.write(i_s[i], j_s[i], values[i].get(), style1)
            print(values[i].get())
        else:
            print(values[i].get())
            wsheet.write(i_s[i], j_s[i], float(values[i].get()), style_float)
    wbook.save('.\\example.xls')
...
def export_excel():
    lg.save_excel()
    excel_new = lg.file_export.get()
    src_file = '.\\example.xls'
    dst_file = excel_new
    lg.pj_file = excel_new.split('.xls')[0] + '.fbi'
...
def peijing_l():
    lg.save_fbi()
    file = lg.file_fbi.get()
    with codecs.open(file, 'w', "GBK") as f:
        words = '以下为箱型桥成图程序输入数据\n*****绘图方式*****\n孔跨类型（1为单孔，2为双孔）:{}\n配筋方式（1为自动分辨，2为大孔径方式，3为小孔径方式）:1\n\n*****线路资料*****\n小里程侧名:{}\n大里程侧名:{}\n中心里程（m）:{}\n中心高程（m）:{}\n线路坡度（‰）:{}\n轨底~板顶距离（m）:{}\n*****箱身主体尺寸（单位cm）*****\n主孔径:{}\n边孔径（暂未用）:0\n净高:{}\n斜交角度（゜左偏为+，右偏为-）:{}\n净保护层厚:{}\n横向宽度（垂直线路方向）:{}\n箱身基础厚度:{}\n\n*****箱身细部尺寸（单位cm）*****\n顶板厚度:{}\n底板厚度:{}\n腹板厚度:{} cm\n中腹板厚度（双孔）:{}\n顶板倒角长（正交断面尺寸）:{}\n顶板倒角高:{}\n底板倒角长（正交断面尺寸）:{}\n底板倒角高:{}\n八字翼墙边坡坡率:1.5\n八字翼墙直挡墙高度:{}\n*****钢筋直径（单位吗m）*****\nN1（顶、底板内侧钢筋Φ）:{}\nN2（底板弯起钢筋Φ）:{}\nN3（顶板弯起钢筋Φ）:{}\nN4（顶、底板外侧钢筋Φ）:{}\nN5（腹板外侧钢筋Φ）:{}\nN6（腹板外侧钢筋Φ）:{}\nN7（腹板内侧钢筋Φ）:{}\nN8、N9（顶、底梗斜钢筋Φ）:{}\nN10、N11（顶、底板架立钢筋Φ）16\nN12（横向分布钢筋Φ）:{}\nN12钢筋间距（cm）:{}\n*双孔中腹板钢筋（Φ）:{}\n*双孔底板加强钢筋（Φ）:{}\n*双孔顶板加强钢筋（Φ）:{}\n箱身基础钢筋（Φ）:{}\n已经清空啦~'.format(lg.kongshu.get(),lg.zuo_li.get(),lg.you_li.get(),lg.xiangti_licheng.get(),
                                   lg.guimian_ele.get(),1000*float(lg.guimian_podu.get()),
                                   100*float(lg.guimian_xiangding.get()),100*float(lg.xiangti_kongjin.get()),
                                   100*float(lg.xiangti_jinggao.get()),float(lg.xiangti_theta.get()),
                                   lg.baohu.get(),100*float(lg.xiangti_qiaokuan.get()),100*float(lg.xiangti_jichuhou.get()),
                                   100*float(lg.xiangti_dingban.get()),100*float(lg.xiangti_diban.get()),
                                   100*float(lg.xiangti_bianfu.get()),100*float(lg.xiangti_midfu.get()),
                                   100*float(lg.xiangti_dingdao_c.get()),100*float(lg.xiangti_dingdao_g.get()),
                                   100*float(lg.xiangti_didao_c.get()),100*float(lg.xiangti_didao_g.get()),
                                   100*float(lg.yiqiang_22.get()),lg.N1_1.get(), lg.N2.get(), lg.N3.get(),
                                   lg.N4.get(), lg.N5.get(),lg.N6.get(), lg.N7.get(), lg.N8_9.get(), lg.N12.get(),
                                   lg.jianju_N12.get(), lg.Nmid_7.get(), lg.N16.get(), lg.N17.get(), lg.N_jichu.get())
...
def shuliang_ti():
    lg.ex_excel()
    excel_new1 = lg.excel_export.get()
    workbook = xlrd.open_workbook('.\\shuliang.xls', formatting_info=True)
    wbook = copy(workbook)
    ...
    for i in range(len(lg.shuliang)):
        wsheet.write(i+1, 4,lg.shuliang[i], style1)
        print(lg.shuliang[i])
    wbook.save(excel_new1)
...
lg.btn_export.configure(command=export_excel)
lg.btn_exzample.configure(command=exzample_input)
lg.btn_ok1.configure(command=read_zhuxiang)
lg.btn_shuliang.configure(command=shuliang_ti)
lg.btn_pj.configure(command=peijing_l)
lg.btn_ok.configure(command=file_shuru)
lg.mainloop()
